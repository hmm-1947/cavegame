...
import static org.lwjgl.vulkan.VK10.VK_DYNAMIC_STATE_DEPTH_BIAS;
...
public class VulkanShadowPipeline {
    ...
    public void create(VkDevice device, long renderPass) {
        ...
        ByteBuffer vertShader = VulkanUtils.loadShader("/shaders/shadow.vert.spv");
        ByteBuffer fragShader = VulkanUtils.loadShader("/shaders/shadow.frag.spv");
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (vkCreatePipelineLayout(device, layoutInfo, null, pPipelineLayout) != VK_SUCCESS) {
                throw new RuntimeException("Failed to create shadow pipeline layout.");
            }
            ...
            if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, pipelineInfo, null, pPipeline) != VK_SUCCESS) {
                throw new RuntimeException("Failed to create shadow pipeline.");
            }
    ...
    private long createShaderModule(VkDevice device, ByteBuffer code) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (vkCreateShaderModule(device, createInfo, null, pShaderModule) != VK_SUCCESS) {
                throw new RuntimeException("Failed to create shadow shader module.");
            }
